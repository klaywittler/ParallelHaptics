function [dJ1] = derivative_deltaInvJacobian(u)
...
%   Detailed explanation goes here
    ...
    % Jacobian
    Jtheta = diag([s(:,1)'*b(:,1) s(:,2)'*b(:,2) s(:,3)'*b(:,3)]);
    Jx = -[s(:,1)'; s(:,2)'; s(:,3)'];
    
    dJthetaInv = diag([(ds(:,1)'*b(:,1)+s(:,1)'*db(:,1))/(s(:,1)'*b(:,1)) (ds(:,2)'*b(:,2)+s(:,2)'*db(:,2))/(s(:,2)'*b(:,2)) (ds(:,3)'*b(:,3)+s(:,3)'*db(:,3))/(s(:,3)'*b(:,3))]);
    dJx = -[ds(:,1)'; ds(:,2)'; ds(:,3)'];
    
    dJ1 = dJthetaInv*Jx + Jtheta\dJx;
